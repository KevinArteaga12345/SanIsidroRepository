...
const CartPage = () => {
  ...
  const handleContinueToPayment = () => {
    ...
    if (appliedPromo && appliedPromo.montoMinimo && subtotal < appliedPromo.montoMinimo) {
      setToast({
        message: `Monto mínimo requerido: S/ ${appliedPromo.montoMinimo}`,
        type: 'error'
      });
      return;
    }
  ...
  // Validar requisitos de la promoción
  const validatePromoRequirements = (promo) => {
    // 1. Validar monto mínimo
    if (promo.montoMinimo && promo.montoMinimo > 0) {
      if (subtotal < promo.montoMinimo) {
        return {
          valid: false,
          message: `Monto mínimo requerido: S/ ${promo.montoMinimo.toFixed(2)}`
        };
      }
    }

    // 2. Validar tipo de promoción
    if (promo.tipoPromocion === 'producto') {
      // Verificar que haya productos aplicables en el carrito
      const applicableItems = cartItems.filter(item => 
        promo.productosAplicables && promo.productosAplicables.includes(item.id)
      );

      if (applicableItems.length === 0) {
        return {
          valid: false,
          message: 'Cupón no disponible para los productos en tu carrito'
        };
      }

      // 3. Validar cantidad mínima de productos aplicables
      const totalQuantity = applicableItems.reduce((sum, item) => {
        const qty = parseInt(item.quantity || item.cantidad) || 0;
        return sum + qty;
      }, 0);
      
      console.log('Validación cantidad:', {
        cantidadMinima: promo.cantidadMinima,
        totalQuantity: totalQuantity,
        applicableItems: applicableItems.map(i => ({ nombre: i.nombre, qty: i.quantity || i.cantidad }))
      });
      
      if (promo.cantidadMinima && totalQuantity < promo.cantidadMinima) {
        return {
          valid: false,
          message: `Cantidad mínima requerida: ${promo.cantidadMinima} unidades del producto aplicable`
        };
      }
    } else {
      // Promoción general - validar cantidad total
      ...
      if (promo.cantidadMinima && totalQuantity < promo.cantidadMinima) {
        return {
          valid: false,
          message: `Cantidad mínima requerida: ${promo.cantidadMinima} productos`
        };
      }
    }
